...
const CreateAssignment = () => {
  ...
  const adjustCounter = (id: string, field: 'count' | 'marks', delta: number) => {
    ...
    <div className="min-h-screen bg-[#F3F4F6]/30 text-[#2D2D2D] p-2 lg:p-2">
      ...
      <header className="max-w-5xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-xl font-bold tracking-tight">Create Assignment</h1>
        </div>
        <p className="text-gray-400 text-sm mb-6">Set up a new assignment for your students</p>
...
<div className="flex items-center gap-3 w-full">
  ...
  />
      ...
      {/* Main Form Card */}
      <main className="max-w-5xl mx-auto bg-[#F3F4F6]/90 rounded-[40px] shadow-sm border border-gray-100 p-6 lg:p-10 mb-8">
        <div className="mb-10">
          <h2 className="text-2xl font-bold mb-1">Assignment Details</h2>
          <p className="text-gray-400 text-sm">Basic information about your assignment</p>
        </div>

        {/* Upload Zone */}
        <div className="border-2 border-dashed border-gray-200 rounded-[32px] p-12 mb-5 flex flex-col items-center justify-center text-center group hover:border-gray-300 transition-colors cursor-pointer">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
            <CloudUpload className="text-black" size={38} />
          </div>
          <p className="text-lg font-semibold mb-1">Choose a file or drag & drop it here</p>
          <p className="text-gray-400 text-light mb-6 uppercase tracking-wider">JPEG, PNG, upto 10MB</p>
        ...
        <p className="text-center text-gray-500 text-lg font-medium mb-12">Upload images of your preferred document/image</p>
        ...
        <div className="mb-12">
          ...
          <div className="relative max-w-full">
            ...
            />
            <Image src="/calendar-icon.svg" alt="Calendar" className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-400" height={30} width={30}/>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="grid grid-cols-12 gap-4 mb-4 px-2">
            <div className="col-span-6 font-bold tracking-widest text-black block text-sm mb-3">Question Type</div>
            <div className="col-span-3 text-sm font-bold tracking-widest text-black text-center">No. of Questions</div>
            <div className="col-span-3 text-sm font-bold tracking-widest text-black text-center">Marks</div>
          </div>
  ...
  <DropdownMenu>
    ...
    <DropdownMenuTrigger asChild>
      <button
        className="w-full flex items-center justify-between bg-white border border-gray-200 rounded-full px-6 py-4 focus:outline-none focus:border-gray-300 font-semibold text-sm transition-all"
      >
        <span>{row.type}</span>
    ...
    >
      {QUESTION_TYPES.map((t) => (
          ...
          {t}
                ...
                <div className="col-span-3 flex items-center justify-center">
                  <div className="flex items-center bg-gray-50 border border-gray-100 rounded-full p-1">
                    ...
                    <span className="w-12 text-center font-bold text-sm">{row.count}</span>
                    <button 
                      onClick={() => adjustCounter(row.id, 'count', 1)}
                      className="w-10 h-10 flex items-center justify-center text-gray-300 hover:text-gray-600"
                    >
                ...
                <div className="col-span-3 flex items-center justify-center">
                  <div className="flex items-center bg-gray-50 border border-gray-100 rounded-full p-1">
                    ...
                    <span className="w-12 text-center font-bold text-sm">{row.marks}</span>
                    <button 
                      onClick={() => adjustCounter(row.id, 'marks', 1)}
                      className="w-10 h-10 flex items-center justify-center text-gray-300 hover:text-gray-600"
                    >
        ...
        <div className="flex flex-col items-end gap-2 mb-8 px-2">
          <p className="text-lg text-black">Total Questions : <span className="text-[#2D2D2D] ml-1">{totalQuestions}</span></p>
          <p className="text-lg text-black">Total Marks : <span className="text-[#2D2D2D] ml-1">{totalMarks}</span></p>
        </div>

        {/* Additional Info */}
        <div className="">
          <label className="block text-md font-bold mb-4">Additional Information (For better output)</label>
